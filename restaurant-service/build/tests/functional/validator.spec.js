...
exports.runAllValidatorTests = void 0;
...
async function runAllValidatorTests() {
    ...
    const createRestaurantSchema = schema.create({
        ...
        status: schema.enum.optional(Object.values(Status_1.RestaurantStatus)),
    ...
    if (!res1Err)
        throw new Error('CreateRestaurantValidator should fail on invalid email');
    ...
    const updateStatusSchema = schema.create({
        status: schema.enum(['ENABLED', 'DISABLED']),
    });
    const statusRes = await validator.validate({ schema: updateStatusSchema, data: { status: 'ENABLED' } });
    if (statusRes.status !== 'ENABLED')
        throw new Error('UpdateRestaurantStatusValidator failed for ENABLED');
    let statusErr = false;
    try {
    ...
    if (!statusErr)
        throw new Error('UpdateRestaurantStatusValidator should REJECT DELETED status');
    ...
    const createAddressSchema = schema.create({
        ...
        latitude: schema.number.optional([rules.range(-90, 90)]),
        longitude: schema.number.optional([rules.range(-180, 180)]),
        status: schema.enum.optional(Object.values(Status_1.AddressStatus)),
    });
    ...
    const createCategorySchema = schema.create({
        name: schema.string({ trim: true }, [rules.maxLength(255)]),
        description: schema.string.optional({ trim: true }, [rules.maxLength(1000)]),
        status: schema.enum.optional(Object.values(Status_1.CategoryStatus)),
    });
    const res5 = await validator.validate({ schema: createCategorySchema, data: { name: 'Pizzas', description: 'Wood-fired pizzas' } });
    if (res5.name !== 'Pizzas')
        throw new Error('CreateCategoryValidator failed');
    ...
    if (res6.description !== 'Updated desc')
        throw new Error('UpdateCategoryValidator failed');
    ...
    const createMenuItemSchema = schema.create({
        ...
        status: schema.enum.optional(Object.values(Status_1.MenuItemStatus)),
    });
    const validMenuItem = {
    ...
    if (!res7Err)
        throw new Error('CreateMenuItemValidator should reject invalid UUID category_id');
    ...
    if (res8.price !== 15.99 || res8.is_vegetarian !== true)
        throw new Error('UpdateMenuItemValidator failed');
    ...
    if (res9.is_available !== false)
        throw new Error('UpdateAvailabilityValidator failed');
    let res9Err = false;
    try {
    ...
    if (!res9Err)
        throw new Error('UpdateAvailabilityValidator should reject non-boolean values');
    ...
    const res10 = await validator.validate({ schema: paginationSchema, data: { page: 2, limit: 50, search: 'burger', status: Status_1.RestaurantStatus.ENABLED } });
